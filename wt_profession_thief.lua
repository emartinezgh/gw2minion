-- This file contains thief specific combat routines
...
wt_profession_thief.RestHealthLimit = 70

-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------
-- NeedHeal Check
wt_profession_thief.c_heal_action = inheritsFrom(wt_cause)
wt_profession_thief.e_heal_action = inheritsFrom(wt_effect)

function wt_profession_thief.c_heal_action:evaluate()
	return (Player.health.percent < 50 and not Player:IsSpellOnCooldown(GW2.SKILLBARSLOT.Slot_6))
end
wt_profession_thief.e_heal_action.usesAbility = true

function wt_profession_thief.e_heal_action:execute()
...
wt_profession_thief.c_MoveCloser = inheritsFrom(wt_cause)
wt_profession_thief.e_MoveCloser = inheritsFrom(wt_effect)

function wt_profession_thief.c_MoveCloser:evaluate()
...
function wt_profession_thief.e_MoveCloser:execute()
...
------------------------------------------------------------------------------
-- Update Weapon Data 
wt_profession_thief.c_update_weapons = inheritsFrom(wt_cause)
wt_profession_thief.e_update_weapons = inheritsFrom(wt_effect)

function wt_profession_thief.c_update_weapons:evaluate()
	wt_profession_thief.MHweapon = Inventory:GetEquippedItemBySlot(GW2.EQUIPMENTSLOT.MainHandWeapon)
	wt_profession_thief.OHweapon = Inventory:GetEquippedItemBySlot(GW2.EQUIPMENTSLOT.OffHandWeapon)
	return false
end

function wt_profession_thief.e_update_weapons:execute()	
end
...
if ( wt_profession_thief.professionID > -1 and wt_profession_thief.professionID == Player.profession) then
	...
	-- Our C & E´s for Thief combat:
	local ke_heal_action = wt_kelement:create("heal_action",wt_profession_thief.c_heal_action,wt_profession_thief.e_heal_action, 100 )
		wt_core_state_combat:add(ke_heal_action)
		
	local ke_MoveClose_action = wt_kelement:create("Move closer",wt_profession_thief.c_MoveCloser,wt_profession_thief.e_MoveCloser, 75 )
		wt_core_state_combat:add(ke_MoveClose_action)

	local ke_Update_weapons = wt_kelement:create("UpdateWeaponData",wt_profession_thief.c_update_weapons,wt_profession_thief.e_update_weapons, 55 )
		wt_core_state_combat:add(ke_Update_weapons)
		
		
	local ke_Attack_default = wt_kelement:create("Attackdefault",wt_profession_thief.c_attack_default,wt_profession_thief.e_attack_default, 45 )
